...
def fetch_flat_patches():
    patches = [f for f in os.listdir('.') if f.endswith('.spec')]
    patches_contents = []
    for patch in patches:
        patches_contents.append((patch, file(patch).read()))
...
def set_patches_on_mirror(msf, sfgerrit, name, mirror,
                          patches, version, workdir):
    with cdir(os.path.join(workdir, name)):
        try:
...
def project_import(cmdargs, workdir, rdoinfo):
    ...
    try:
        patches, vers = import_distgit(msf, sfgerrit,
    ...
    set_patches_on_mirror(msf, sfgerrit, name, mirror, patches,
                          version, workdir)
