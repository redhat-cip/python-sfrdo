...
def projects_status(cmdargs, workdir, rdoinfo):
    ...
    print "Inconsistent: %s : %s" % (len(inconsistent), ", ".join(inconsistent))
    notimported = [p for p, s in status.items() if s == 0]
    print "Not imported: %s : %s" % (len(notimported), ", ".join(notimported))
...
def check_project_status(sfprojects, name):
    ...
    if not name in sfprojects and not sfdistgit in sfprojects:
